...
# and `pip install intermap` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "intermap", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n intermap -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "intermap", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "intermap", "YOUR_CONSOLE_SCRIPT"]
