...
from intermap.intervis.app.icsv import interaction_priority
...
class HeatMap(BaseTab):
    ...
    def _create_single_interaction_heatmap(self, interaction,
        ...
        text_matrix = np.where(
            mask,
            np.where(prevalence_values > 0,
                     np.round(prevalence_values, 1).astype(str), ''),
            ''
        )
    ...
    def create_heatmap_plot(self, axisx, axisy):
        ...
        # Add base heatmap
        base_trace = self.create_base_traces()
